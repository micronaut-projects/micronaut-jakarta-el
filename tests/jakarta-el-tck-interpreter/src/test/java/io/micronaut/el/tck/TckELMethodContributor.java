...
public final class TckELMethodContributor implements ELMethodContributor {
    ...
    /**
     * The {@code Comparable} and {@code Object} methods the lambda and coercion tests call.
     *
     * <p>The bridge {@code compareTo(Object)} is registered once, on {@code Comparable} itself, since the
     * registry merges what an interface declares into every type implementing it. An argument that is not of
     * the type of the receiver is assignable to the bridge but only coercible to the typed overload, so the
     * bridge is selected and the coercion of the section 1.23 does not apply, exactly as it does not when the
     * method is found reflectively; the cast inside it then fails the way the synthetic bridge of the class
     * does.
     */
    private static void comparisons(ELMethodRegistry registry) {
        registry.method(Integer.class, "compareTo", int.class, Integer.class, Integer::compareTo)
            ...
            .method(Comparable.class, "compareTo", int.class, new Class<?>[]{Object.class}, false,
                (context, base, arguments) -> compareToBridge(base, arguments[0]))
    ...
    private static int compareToBridge(@Nullable Object base, @Nullable Object first) {
        return ((Comparable<Object>) base).compareTo(first);
    }
