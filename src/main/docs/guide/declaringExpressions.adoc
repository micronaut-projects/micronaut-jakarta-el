...
time.
<3> `functions` exposes the public static methods of a class as functions, optionally under a namespace prefix,
which a method can override with `@ELFunction`.
...
static methods.
<2> `@ELFunction` can give a function another name, and a namespace prefix of its own.
...
<3> A static method is a function as well, invoked directly.

snippet::example.PricingExpressions[project-base="doc-examples/example", title="PricingExpressions"]

<1> The bean is listed in the environment like any class of functions; the prefix comes from the methods.
