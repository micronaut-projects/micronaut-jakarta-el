...
<1> An ordinary bean: nothing lists it anywhere. A function declared with `@ELFunction` is found by the
expressions of the module declaring it, and of every module depending on it, because the processor indexes the
classes declaring functions. The module declaring them has to run the annotation processor.
