...
the declaration, changeable without touching the surrounding code, and — with this module — compiled and
...
snippet::example.BusinessRules[project-base="doc-examples/example", tags="register", indent=0]
...
snippet::example.BusinessRules[project-base="doc-examples/example", tags="deposit", indent=0]
...
----
@FeatureFlag("#{ customer.plan == 'PRO' or fn:betaTester(customer.id) }")
fun newDashboard(customer: Customer): Dashboard { ... }
----
...
----
@FeatureFlag("#{ customer.plan == 'PRO' or fn:betaTester(customer.id) }")
Dashboard newDashboard(Customer customer) { ... }
----
...
----
@FeatureFlag("#{ customer.plan == 'PRO' or fn:betaTester(customer.id) }")
def new_dashboard(self, customer: Customer) -> Dashboard: ...
----
...
`jakarta.el.ExpressionFactory`; everything an application declares in source stays compiled. The rule source
must be fully trusted because runtime expressions are executable application code, not sandboxed data.
...
NOTE: The library is written in Java whatever the language of the application using it: the annotation processor
it declares runs inside the compiler, for a Java, Kotlin, Groovy or Python build alike, so the sources of this
section are shown in Java only. The beans declaring the expressions and the interceptor evaluating them are
application code, and are shown in every language.
...
<3> The compiled expressions carry the expected types the processor declared; in Java, Kotlin and Python the
registry of the class also exposes them under the names the annotation gave them.
