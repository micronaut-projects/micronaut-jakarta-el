...
public final class ELMethods {
     ...
     */
    // the coercion of the section 1.23 for every parameter, with the variable arity ones packed: one pass
    // over the arguments, whose branches are the cases the specification distinguishes
    ...
    public static Object[] coerceArguments(ELContext context,
        ...
        if (varArgs) {
            ...
            if (count == parameterTypes.length && arguments[fixed] != null
                ...
                Object varargs = ELArray.newInstance(componentType, Math.max(0, count - fixed));
     ...
     */
    // the category a candidate falls in, section 1.6: the worst of its parameters decides, so every
    // parameter is examined in one pass and the branches are the categories themselves
    @SuppressWarnings("java:S3776")
     ...
     *
     * <p>Java would rule a primitive parameter out for a null and prefer the reference overload. Expressly
     * and the Jasper implementation both select the primitive one, so this does too: the differential fuzz
     * test holds the three to the same answer.</p>
     */
     ...
     * @return The failure
     */
    public static MethodNotFoundException notFound(Class<?> type, String name, int count) {
