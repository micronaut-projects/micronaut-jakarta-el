...
public final class ELArray {
    ...
    /**
     * Creates an array of the given component type, without {@code java.lang.reflect.Array}.
     *
     * <p>A primitive component type reaches the JVM array creation of that type directly. A reference one goes
     * through {@link java.util.Arrays#copyOf(Object[], int, Class)}, which builds the array of the requested
     * type and leaves its elements null.</p>
     *
    ...
    public static Object newInstance(Class<?> componentType, int length) {
        ...
        return Arrays.copyOf(EMPTY, length, (Class<? extends Object[]>) componentType.arrayType());
    }
    ...
    private static short shortValue(@Nullable Object value) {
        // one widening conversion per type the JVM allows into a short[]
        return switch (value) {
            case Byte widened -> widened;
            case Short widened -> widened;
            case null, default -> throw incompatible(value, short.class);
        };
    }

    private static int intValue(@Nullable Object value) {
        // one widening conversion per type the JVM allows into an int[]
        return switch (value) {
            case Byte widened -> widened;
            case Short widened -> widened;
            case Character widened -> widened;
            case Integer widened -> widened;
            case null, default -> throw incompatible(value, int.class);
        };
    }

    private static long longValue(@Nullable Object value) {
        // one widening conversion per type the JVM allows into a long[]
        return switch (value) {
            case Byte widened -> widened;
            case Short widened -> widened;
            case Character widened -> widened;
            case Integer widened -> widened;
            case Long widened -> widened;
            case null, default -> throw incompatible(value, long.class);
        };
    }

    private static float floatValue(@Nullable Object value) {
        // one widening conversion per type the JVM allows into a float[]
        return switch (value) {
            case Byte widened -> widened;
            case Short widened -> widened;
            case Character widened -> widened;
            case Integer widened -> widened;
            case Long widened -> widened;
            case Float widened -> widened;
            case null, default -> throw incompatible(value, float.class);
        };
    }

    private static double doubleValue(@Nullable Object value) {
        // one widening conversion per type the JVM allows into a double[]
        return switch (value) {
            case Byte widened -> widened;
            case Short widened -> widened;
            case Character widened -> widened;
            ...
            case Long widened -> widened;
            case Float widened -> widened;
            case Double widened -> widened;
            case null, default -> throw incompatible(value, double.class);
        };
    }
