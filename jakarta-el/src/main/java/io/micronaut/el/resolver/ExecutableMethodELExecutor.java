...
public final class ExecutableMethodELExecutor extends ELResolver implements ELMethodExecutor {
     ...
     */
    // the executable method is not serializable, and never reaches a stream: this executor binds no
    // function, so nothing of it is held in the state an expression writes out, and the method is not
    // reusable, so a call site resolves it again rather than keeping it
