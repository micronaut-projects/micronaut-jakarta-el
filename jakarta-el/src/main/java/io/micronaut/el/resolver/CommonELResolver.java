...
public final class CommonELResolver extends ELResolver implements ELMethodExecutor {
    ...
    private static ELMethod resolveString(String method,
        ...
        return switch (method) {
            ...
            case IS_EMPTY -> arity == 0 ? CommonMethod.STRING_IS_EMPTY : null;
            ...
            case TO_STRING -> arity == 0 ? CommonMethod.STRING_TO_STRING : null;
    ...
    private static ELMethod resolveCollection(Collection<?> collection,
        ...
        return switch (method) {
            ...
            case IS_EMPTY -> arity == 0 ? CommonMethod.COLLECTION_IS_EMPTY : null;
            ...
            case INDEX_OF -> collection instanceof List<?> list && arity == 1
    ...
    private static ELMethod resolveMap(String method,
        ...
        return switch (method) {
            ...
            case IS_EMPTY -> arity == 0 ? CommonMethod.MAP_IS_EMPTY : null;
    ...
    private static ELMethod resolveStatic(Class<?> type,
        ...
        if (type == Integer.class) {
            return integerMethod(method, argumentTypes, arguments, arity);
        }
        if (type == Math.class && arity == 2) {
            return mathMethod(method, argumentTypes, arguments);
        }
        return null;
    }

    /**
     * The static methods of {@code Integer} an expression reaches directly.
     */
    @Nullable
    private static ELMethod integerMethod(String method,
                                          Argument<?> @Nullable [] argumentTypes,
                                          Object[] arguments,
                                          int arity) {
        return switch (method) {
            case VALUE_OF -> arity == 1 && isString(argumentTypes, arguments)
    ...
    /**
     * The overload of {@code Math.max} or {@code Math.min} the call selects: what the expression declared
     * decides it, and the classes the arguments happen to have only when it declared nothing.
     */
    @Nullable
    private static ELMethod mathMethod(String method,
                                       Argument<?> @Nullable [] argumentTypes,
                                       Object[] arguments) {
        ELMethod declared = mathByDeclaredTypes(method, argumentTypes);
        return declared != null ? declared : mathByArgumentTypes(method, arguments);
    }

    /**
     * The overload the parameter types a method expression declared select, section 1.6, which is read before
     * the classes of the arguments. The length says whether there are two of them to select from, not the
     * array being present.
     */
    @Nullable
    private static ELMethod mathByDeclaredTypes(String method, Argument<?> @Nullable [] argumentTypes) {
        ...
        if (isType(type0, int.class) && isType(type1, int.class)) {
            return mathOverload(method, CommonMethod.MATH_MAX_INT, CommonMethod.MATH_MIN_INT);
        }
        if (isType(type0, long.class) && isType(type1, long.class)) {
            return mathOverload(method, CommonMethod.MATH_MAX_LONG, CommonMethod.MATH_MIN_LONG);
        }
        if (isType(type0, double.class) && isType(type1, double.class)) {
            return mathOverload(method, CommonMethod.MATH_MAX_DOUBLE, CommonMethod.MATH_MIN_DOUBLE);
        }
        if (isType(type0, float.class) && isType(type1, float.class)) {
            return mathOverload(method, CommonMethod.MATH_MAX_FLOAT, CommonMethod.MATH_MIN_FLOAT);
        }
        return null;
    }

    /**
     * The overload the arguments themselves select, for a call that declared no parameter types.
     */
    @Nullable
    private static ELMethod mathByArgumentTypes(String method, Object[] arguments) {
        ...
        if (arguments[0] instanceof Float && arguments[1] instanceof Float) {
            return mathOverload(method, CommonMethod.MATH_MAX_FLOAT, CommonMethod.MATH_MIN_FLOAT);
        }
        return null;
    }
    ...
    @Nullable
    private static ELMethod mathOverload(String method, CommonMethod maximum, CommonMethod minimum) {
        return switch (method) {
            case MAX -> maximum;
            case MIN -> minimum;
            default -> null;
    ...
    /**
     * Whether the single argument is a string. What a method expression declared decides it, section 1.6, and
     * the class the argument happens to have only when it declared nothing.
     */
    private static boolean isString(Argument<?> @Nullable [] argumentTypes, Object[] arguments) {
        if (argumentTypes != null && argumentTypes.length == 1) {
            return argumentTypes[0].getType() == String.class;
        }
    ...
    /**
     * The argument coerced to the declared parameter type of the method, which a null never reaches: a
     * primitive parameter has no value to stand for one.
     *
     * @param context   The context
     * @param value     The argument
     * @param primitive The declared parameter type
     * @param wrapper   The type the coercion returns
     * @param <T>       The type the coercion returns
     * @return The coerced argument
     */
    private static <T> T argument(ELContext context,
                                  @Nullable Object value,
                                  Class<?> primitive,
                                  Class<T> wrapper) {
        T coerced = wrapper.cast(ELSupport.coerceToType(context, value, primitive));
        if (coerced == null) {
            throw new ELException("Cannot coerce a null value to " + primitive.getName());
        }
        return coerced;
    }
    ...
    private enum CommonMethod implements ELMethod {
        ...
        STRING_IS_EMPTY(IS_EMPTY, boolean.class),
        ...
        STRING_TO_STRING(TO_STRING, String.class),
        ...
        COLLECTION_IS_EMPTY(IS_EMPTY, boolean.class),
        ...
        LIST_INDEX_OF(INDEX_OF, int.class, Object.class),
        LIST_LAST_INDEX_OF("lastIndexOf", int.class, Object.class),
        MAP_SIZE(SIZE, int.class),
        MAP_IS_EMPTY(IS_EMPTY, boolean.class),
        ...
        INTEGER_VALUE_OF_STRING(VALUE_OF, Integer.class, String.class),
        INTEGER_VALUE_OF_INT(VALUE_OF, Integer.class, int.class),
        ...
        public Object invoke(ELContext context, @Nullable Object base, Object @Nullable [] arguments) {
            ...
            return switch (this) {
                ...
                // the two overloads differ in what they take, so they differ in how the argument is coerced:
                // `valueOf(String)` parses the string, where `valueOf(int)` takes the number itself
                case INTEGER_VALUE_OF_STRING -> Integer.valueOf(ELSupport.coerceToString(values[0]));
                case INTEGER_VALUE_OF_INT -> Integer.valueOf(intArgument(context, values[0]));
                case INTEGER_TO_HEX_STRING -> Integer.toHexString(intArgument(context, values[0]));
                // the arguments reach the declared parameter type through the coercion of the section 1.23,
                // the way they do for every other direct method: the selection can have been made from the
                // parameter types a method expression declared rather than from the values themselves
                case MATH_MAX_INT -> Math.max(intArgument(context, values[0]), intArgument(context, values[1]));
                case MATH_MIN_INT -> Math.min(intArgument(context, values[0]), intArgument(context, values[1]));
                case MATH_MAX_LONG -> Math.max(argument(context, values[0], long.class, Long.class),
                    argument(context, values[1], long.class, Long.class));
                case MATH_MIN_LONG -> Math.min(argument(context, values[0], long.class, Long.class),
                    argument(context, values[1], long.class, Long.class));
                case MATH_MAX_DOUBLE -> Math.max(argument(context, values[0], double.class, Double.class),
                    argument(context, values[1], double.class, Double.class));
                case MATH_MIN_DOUBLE -> Math.min(argument(context, values[0], double.class, Double.class),
                    argument(context, values[1], double.class, Double.class));
                case MATH_MAX_FLOAT -> Math.max(argument(context, values[0], float.class, Float.class),
                    argument(context, values[1], float.class, Float.class));
                case MATH_MIN_FLOAT -> Math.min(argument(context, values[0], float.class, Float.class),
                    argument(context, values[1], float.class, Float.class));
            };
        }
