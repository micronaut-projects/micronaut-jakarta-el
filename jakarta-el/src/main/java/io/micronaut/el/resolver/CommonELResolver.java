...
public final class CommonELResolver extends ELResolver implements ELMethodExecutor {
    ...
    private static ELMethod resolveStatic(Class<?> type,
        ...
        if (type == Math.class && arity == 2) {
            // the parameter types a method expression declared select the overload, section 1.6, so they
            // are read before the classes the arguments happen to have. The length is what says whether
            // there are two of them to select from, not the array being present
